...
import { validRuntime } from 'core/catalog/index';
...
abstract class BaseCatalogItem implements CatalogItem {
    ...
    protected mergeVersionPoms(runtime: Runtime, targetPom = 'pom.xml') {
        const sourcePom = `merge/pom.${validRuntime(runtime).version}.xml`;
        try {
            accessSync(path.join(this.sourceDir, sourcePom));
            return this.mergePoms(sourcePom, targetPom);
        } catch (ex) {
